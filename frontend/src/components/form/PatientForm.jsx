...
const PatientForm = () => {
  const [currentStep, setCurrentStep] = useState(1);
  const [showModal, setShowModal] = useState(false);
  const [submissionError, setSubmissionError] = useState(''); // ✅ global error
  const [errors, setErrors] = useState({});
  const [formData, setFormData] = useState({
    deviceType: '',
    location: '',
  ...
  const handleNext = () => {
    ...
    if (currentStep === 7) {
      if (formData.usesInternetHealth === 'Yes') return setCurrentStep(8);
      if (formData.usesInternetHealth === 'No') return setCurrentStep(9);
    }
  ...
  const handleBack = () => {
    ...
    if ((currentStep === 8 || currentStep === 9)) return setCurrentStep(7);
    setCurrentStep(prev => Math.max(prev - 1, 1));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateCurrentStep()) {
      // ✅ Validate email and show error inline
      if (!formData.email || !/\S+@\S+\.\S+/.test(formData.email)) {
      ...
      setShowModal(false);
      setSubmissionError(''); // reset global error
      ...
      try {
        ...
        console.error('Submission error', err.response?.data || err.message);
        setSubmissionError('Something went wrong. Please try again.'); // ✅ global error
      }
    }
  };
  ...
  return (
    <div className='survey-body'>
      <form onSubmit={handleSubmit} className="survey-form">
        ...
        <div className="survey-header">
          <h1>Patient Experience Survey</h1>
          <p>Your experience and insights are crucial! We are developing a new mobile-friendly system to help patients in Africa more easily find specialist care and discover relevant clinical trials.</p>
          <p>Your responses will directly inform how we build this system to meet your needs. All your answers will be kept confidential.</p>
        </div>
        ...
        {/* ✅ Show submission error inside form */}
        {submissionError && (
          <div className="error-message">
            <p>{submissionError}</p>
          </div>
